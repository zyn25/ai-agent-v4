...
export class PositionRepository extends BaseRepository {
  ...
  create(p) { return this.db.prepare('INSERT INTO positions (id,pair,side,entry_price,quantity,leverage,stop_loss,take_profit,status,ai_confidence,ai_decision,strategy_version,open_time) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)').run(p.id,p.pair,p.side,p.entry_price,p.quantity,p.leverage,p.stop_loss,p.take_profit,p.status,p.ai_confidence,p.ai_decision,p.strategy_version,p.open_time); }

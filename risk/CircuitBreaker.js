export class CircuitBreaker {
  #config; #logger; #db; #paused=false; #reason=''; #pauseTime=null;
  ...
  async check() {
    ...
    if(p.equity&&p.equity>0) { const dd=Math.abs((p.balance-p.equity)/p.balance)*100; if(dd>=this.#config.risk.maxDrawdown) { this.#pause('Max drawdown'); return{allowed:false,reason:'Max drawdown'}; } }
    if(p.daily_pnl<0&&p.balance>0) { const pct=Math.abs(p.daily_pnl/p.balance)*100; if(pct>=this.#config.risk.maxDailyLoss) { this.#pause('Daily loss limit'); return{allowed:false,reason:'Daily loss limit'}; } }
    if(p.weekly_pnl<0&&p.balance>0) { const pct=Math.abs(p.weekly_pnl/p.balance)*100; if(pct>=this.#config.risk.maxWeeklyLoss) { this.#pause('Weekly loss limit'); return{allowed:false,reason:'Weekly loss limit'}; } }

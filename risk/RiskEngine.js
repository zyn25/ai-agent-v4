...
export class RiskEngine {
  ...
  constructor(config, logger, db) { this.#config = config; this.#logger = logger; this.#database = db; this.#breaker = new CircuitBreaker(config, logger, db); this.#sizer = new PositionSizer(config); }
  async canTrade() {
    ...
    if (open >= this.#config.risk.maxOpenPositions) return { allowed: false, reason: 'Max positions' };
    return { allowed: true };
  }
  ...
  shouldBreakEven(cur, entry, be, side) { return side === 'long' ? cur >= be : cur <= be; }
  getTrailingStop(cur, atr, side) { const d = atr * this.#config.risk.trailingStopATR; return side === 'long' ? cur - d : cur + d; }

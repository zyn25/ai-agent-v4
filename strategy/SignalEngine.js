...
export class SignalEngine {
  ...
  #calc(d, w) {
    ...
    const rv = RSIIndicator.calculate(closes, ind.rsiPeriod), ri = RSIIndicator.interpret(rv[rv.length-1], ind.rsiOverbought, ind.rsiOversold);
    const mc = MACDIndicator.calculate(closes, ind.macdFast, ind.macdSlow, ind.macdSignal), mi = MACDIndicator.interpret(mc.MACD, mc.signal, mc.histogram);

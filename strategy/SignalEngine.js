...
export class SignalEngine {
  ...
  async analyze(pair) {
    ...
    try {
      ...
      this.#logger.trade('SIGNAL: '+targetPair+' | '+primary+':'+ps.trend+'('+ps.score.toFixed(1)+') | '+secondary+':'+ss.trend+'('+ss.score.toFixed(1)+') | '+tertiary+':'+ts.trend+'('+ts.score.toFixed(1)+') | MTF:'+mtf.toFixed(1)+' | Aligned:'+aligned+' | Threshold:'+threshold);
      ...
      if(confidence<threshold) return {pair:targetPair,side:'neutral',confidence,reason:'Below threshold ('+threshold+'%, got '+confidence.toFixed(1)+'%)'};

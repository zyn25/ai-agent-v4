...
export class MarketFilter {
  ...
  #checkVolatility(highs, lows, closes) {
    ...
    if (atrPct > 5.0) return { pass: false, reason: 'ATR too high (' + atrPct.toFixed(1) + '%)', score: 0 };
  ...
  #checkRanging(closes, highs, lows) {
    ...
    if (range20 < atrPct * 2) return { pass: false, reason: 'Ranging (range:' + range20.toFixed(2) + '% < 2x ATR)', score: 0 };

...
describe('ATRIndicator', () => {
  ...
  const cl = [48.16,48.61,48.75,48.63,48.74,49.03,49.07,49.32,49.91,50.13,49.53,49.50,49.75,50.03,49.61,49.26,50.09,50.39,49.60,49.56,49.91,50.01,49.20,49.81,49.06,48.66,48.09,48.65,48.37,48.53];
  it('calculate ATR', () => { assert.ok(ATRIndicator.calculate(h, l, cl, 14).length > 0); });
  it('positive values', () => { for (const v of ATRIndicator.calculate(h, l, cl, 14)) assert.ok(v > 0); });
  it('correct length', () => { assert.equal(ATRIndicator.calculate(h, l, cl, 14).length, h.length - 14 + 1); });
});
